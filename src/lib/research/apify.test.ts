...
import { apifyTokens, creditFromLimits, errorSnippet, estimateScanUsd, eventChargeUsd, hintOrder, IG_RESULT_USD, isTerminalRunStatus, runCostUsd, scanChargeCap } from "./apify";
import { freeOnly, parseModelJson, providers, isDailyWall, isPerMinuteLimit, retryDelayMs } from "./llm";
import { keyChecks, missingKeyMessage, numberedKeys } from "./keys";
import { usdText } from "./apify";
...
describe("keeping tokens out of anything printed", () => {
  it("takes a token out of an error message", () => {
    expect(errorSnippet("failed for apify_api_notarealtoken")).toBe("failed for [token]");
...
describe("reading tokens out of the environment", () => {
  it("reads a numbered family in order and drops repeats", () => {
    const env = { APIFY_API_TOKEN: "one", APIFY_API_TOKEN_2: "two", APIFY_API_TOKEN_3: "one" } as unknown as NodeJS.ProcessEnv;
    expect(numberedKeys("APIFY_API_TOKEN", env)).toEqual(["one", "two"]);
  });

  it("also accepts the shorter name and a comma separated list", () => {
    const env = { APIFY_TOKENS: "a, b", APIFY_TOKEN: "c" } as unknown as NodeJS.ProcessEnv;
  ...
  it("strips quotes and blanks from a list of keys", () => {
    const env = { APIFY_TOKENS: '"a", \'b\' , , c' } as unknown as NodeJS.ProcessEnv;
    expect(apifyTokens(env)).toEqual(["a", "b", "c"]);
  });
});

describe("saying which key is missing", () => {
  it("names every key Research can use and whether it is set", () => {
    const env = { GROQ_API_KEY: "k" } as unknown as NodeJS.ProcessEnv;
    const checks = keyChecks(env);
    expect(checks.map((c) => c.name)).toEqual(["APIFY_API_TOKEN", "GROQ_API_KEY", "GEMINI_API_KEY", "YOUTUBE_API_KEY"]);
    expect(checks.find((c) => c.name === "GROQ_API_KEY")?.present).toBe(true);
    expect(checks.find((c) => c.name === "APIFY_API_TOKEN")?.present).toBe(false);
  });

  it("writes a message naming the key and where to get one, and never a key itself", () => {
    const message = missingKeyMessage("GEMINI_API_KEY", {} as unknown as NodeJS.ProcessEnv);
...
describe("money in words", () => {
  it("never writes a real cost as free", () => {
    expect(usdText(0.004)).toBe("less than a cent");
    expect(usdText(0.04)).toBe("$0.04");
    expect(usdText(1.2)).toBe("$1.20");
    expect(usdText(0)).toBe("nothing");
    expect(usdText(null)).toBe("not known");
  });
});
...
describe("free providers only", () => {
  ...
  it("never builds a provider from a paid key sitting in the environment", () => {
    const env = { ANTHROPIC_API_KEY: "sk-ant-x", OPENAI_API_KEY: "sk-x", GROQ_API_KEY: "gsk_x" } as unknown as NodeJS.ProcessEnv;
  ...
  it("makes one provider entry per key, so a spent day moves to the next project", () => {
    const env = { GEMINI_API_KEYS: "a,b,c" } as unknown as NodeJS.ProcessEnv;

...
import type { DraftPartKey } from "./types";
...
describe("making a model's report safe", () => {
  ...
  it("throws away a hook rewrite that is too long or claims a result", () => {
    const r = coerceReport(
      { ...good, hook_rewrites: [Array.from({ length: 20 }, () => "alpha bravo charlie delta").join(" "), "I booked 40 jobs from this", "The needle is the problem"] },
      CLEAN,
    )!;

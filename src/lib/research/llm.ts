...
import { geminiKeys, groqKeys, keyList, missingKeyMessage } from "./keys";
...
export async function askJson(
  prompt: string,
  opts: { label: string; temperature?: number; providers?: Provider[]; timeoutMs?: number; quiet?: boolean; deadline?: number },
): Promise<LlmAnswer> {
  const pool = opts.providers ?? providers();
  if (pool.length === 0) throw new MissingKeyError("GROQ_API_KEY", missingKeyMessage("GROQ_API_KEY"));
  let last = "no provider answered";
  let spent = 0;
  for (const p of pool) {
    // A caller with its own clock (a server action has one) stops here rather than starting a call
    // it has no time left to finish.
    if (opts.deadline !== undefined && opts.deadline - Date.now() < 2_000) break;
    for (let attempt = 1; attempt <= 2; attempt++) {
      ...
      if (res.ok) {
        ...
        recordAiCall({ kind: "llm", label: opts.label, provider: p.name, model: p.model, ms });
        return { json, provider: p.name, model: p.model, ms };
      }

...
import { missingKeyMessage, youtubeKey } from "./keys";
...
async function yt<T>(endpoint: Endpoint, params: Record<string, string>): Promise<T> {
  ...
  if (!key) throw new MissingKeyError("YOUTUBE_API_KEY", missingKeyMessage("YOUTUBE_API_KEY"));
  let res: Response;
  try {
  ...
  for (let i = 0; i < channelIds.length; i += 50) {
    ...
    for (const c of data.items ?? []) {
      ...
      channels.push({
        ...
        avatar: bestThumbnail(c.snippet?.thumbnails),

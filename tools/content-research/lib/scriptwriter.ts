...
import { profileIsThin, profileSources, type SourceVideo } from "./adaptation";
...
/** The script as the markdown file that gets saved and read out loud while filming. */
export function scriptMarkdown(args: {
  draft: ScriptDraft;
  client: ClientProfile;
  video: SourceVideo;
  breakdown: Breakdown;
  ...
  const { draft, video, breakdown: b } = args;
  const words = spokenWords(draft.script);
  const lines = [
    `# ${args.client.name}: ${draft.hook}`,
    "",
    `Written ${args.at.toISOString().slice(0, 16).replace("T", " ")} by ${args.model}.`,
    `Borrows the pattern from ${video.creatorName}, ${video.performance}`,
    video.url,
    "",
    "## On screen, first 2 seconds",
    draft.on_screen ?? "(none chosen)",
    "",
    `## Script (${words} spoken words, about ${Math.round((words / 150) * 60)} seconds)`,
    "",
    draft.script,
    "",
    "## Ask",
    ...
    "",
    "## Shots to get",
    ...(draft.shot_list.length ? draft.shot_list.map((s) => `- ${s}`) : ["- (none listed)"]),
    "",
    "## What it borrows",
    b.pattern_you_can_use ? `Pattern: ${b.pattern_you_can_use}` : "Pattern: not recorded",
    b.hook_type ? `Hook type: ${b.hook_type}` : "Hook type: not recorded",
  ];
  if (args.problems.length) {
    lines.push(
      "",
      "## Checks this script still fails",
      "Fix these before it is filmed. The tool could not fix them itself.",
      ...args.problems.map((p) => `- ${p}`),
    );
  }
  if (profileIsThin(args.client)) {
    lines.push(
      "",
      "## The brackets",
      "The client profile is too thin to write real detail from, so anything in [brackets] is a blank",
      "you have to fill in with the client's own answer. Do not guess one.",
    );
  }
  return `${lines.join("\n")}\n`;
}

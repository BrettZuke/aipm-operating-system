...
export async function transcribeUrl(mediaUrl: string, opts: { label: string; quiet?: boolean }): Promise<Transcript> {
  const keys = groqKeys();
  if (keys.length === 0) throw new TranscribeError("No Groq key is set (GROQ_API_KEY), so the words cannot be pulled out");
  ...
  for (const model of WHISPER_MODELS) {
    for (let i = 0; i < keys.length; i++) {
      ...
      for (let attempt = 1; attempt <= 2; attempt++) {
        ...
        if (res.ok) {
          return parseTranscription(await res.json(), slot, model, Date.now() - started);
        }
